...
public class ProductImageController {
    ...
    public Object add(@PathVariable("pid") int pid, @RequestParam("type") String type, MultipartFile image, HttpServletRequest request) throws Exception {
        ...
        if (ProductImageService.type_single.equals(bean.getType())) {
            ...
            ImageUtil.resizeImage(file, 217, 129, f_middle);
        }
        return bean;
    }
    ...
    public String delete(@PathVariable("id") int id, HttpServletRequest request) throws Exception {
        ...
        file.delete();
        if (ProductImageService.type_single.equals(bean.getType())) {
            String image_Folder_small = request.getServletContext().getRealPath("img/productSingle_small");
            String image_Folder_middle = request.getServletContext().getRealPath("img/productSingle_middle");
            File f_small = new File(image_Folder_small, fileName);
            File f_middle = new File(image_Folder_middle, fileName);
            f_small.delete();
            f_middle.delete();
        }
        return null;
